...
class ActivoFijoModel extends Modelo
{
    ...
    public function guardar(array $data, int $userId): int
    {
        ...
        if ($payload['codigo_activo'] === '' || $payload['nombre'] === '' || $payload['id_cuenta_activo'] <= 0 || $payload['id_cuenta_depreciacion'] <= 0) {
            throw new RuntimeException('Complete los campos obligatorios del activo fijo.');
        }
