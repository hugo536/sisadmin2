...
<div class="container-fluid p-4" id="tesoreriaCxcApp">
    <div class="d-flex justify-content-between align-items-center mb-4 fade-in">
        ...
        <a href="<?php echo e(route_url('tesoreria/movimientos')); ?>" class="btn btn-outline-primary shadow-sm fw-semibold">
            <i class="bi bi-clock-history me-1"></i>Movimientos
        </a>
    </div>

    <?php if (!empty($_GET['ok'])): ?><div class="alert alert-success py-2">Cobro registrado correctamente.</div><?php endif; ?>
    <?php if (!empty($_GET['error'])): ?><div class="alert alert-danger py-2"><?php echo e((string) $_GET['error']); ?></div><?php endif; ?>

    <div class="card border-0 shadow-sm mb-3">
        <div class="card-body p-3">
            <form method="get" action="<?php echo e(route_url('tesoreria/cxc')); ?>" class="row g-2 align-items-center">
                <div class="col-md-3">
                ...
                <div class="col-md-3">
                    <select class="form-select bg-light" name="moneda">
                        <option value="">Moneda (todas)</option>
                        <option value="PEN" <?php echo (($filtros['moneda'] ?? '') === 'PEN') ? 'selected' : ''; ?>>PEN</option>
                        <option value="USD" <?php echo (($filtros['moneda'] ?? '') === 'USD') ? 'selected' : ''; ?>>USD</option>
                    </select>
                </div>
                <div class="col-md-3">
                    <select class="form-select bg-light" name="vencimiento">
                        <option value="">Vencimiento (todos)</option>
                        <option value="vencidas" <?php echo (($filtros['vencimiento'] ?? '') === 'vencidas') ? 'selected' : ''; ?>>Solo vencidas</option>
                    </select>
                </div>
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table align-middle mb-0 table-pro" id="tablaTesoreriaCxc">
                    <thead>
                    <tr>
                        <th class="ps-4 text-secondary fw-semibold">Cliente</th>
                        <th class="text-secondary fw-semibold">Documento</th>
                        <th class="text-secondary fw-semibold">Vence</th>
                    ...
                    <?php foreach ($registros as $r): ?>
                        <tr class="border-bottom">
                            <td class="ps-4 fw-semibold text-dark"><?php echo e((string) ($r['cliente'] ?? '')); ?></td>
                            <td>#<?php echo (int) ($r['id_documento_venta'] ?? 0); ?></td>
                            ...
                            <td class="text-end pe-4">
                                <?php if (in_array((string) ($r['estado'] ?? ''), ['ABIERTA','PARCIAL','VENCIDA'], true)): ?>
                                    <button class="btn btn-sm btn-success js-open-cobro" data-id-origen="<?php echo (int) $r['id']; ?>" data-moneda="<?php echo e((string) $r['moneda']); ?>" data-saldo="<?php echo e((string) $r['saldo']); ?>">Registrar cobro</button>
                                <?php endif; ?>
                                <a class="btn btn-sm btn-outline-secondary" href="<?php echo e(route_url('tesoreria/movimientos')); ?>&origen=CXC&id_origen=<?php echo (int) $r['id']; ?>&id_tercero=<?php echo (int) ($r['id_cliente'] ?? 0); ?>">Historial</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
    ...
    <div class="modal-content border-0 shadow">
      <form method="post" action="<?php echo e(route_url('tesoreria/registrar_cobro')); ?>" class="js-form-confirm js-form-monto">
        ...
        <div class="modal-body row g-2">
          ...
          <div class="col-6"><label class="form-label">Moneda</label><input name="moneda" id="cobroMoneda" class="form-control" readonly></div>
          <div class="col-6"><label class="form-label">Saldo</label><input id="cobroSaldo" class="form-control" readonly data-saldo-target="1"></div>
          ...
          <div class="col-6"><label class="form-label">Monto</label><input type="number" step="0.0001" min="0.0001" name="monto" id="cobroMonto" class="form-control" required></div>
          <div class="col-12"><label class="form-label">Referencia</label><input name="referencia" class="form-control"></div>
          <div class="col-12"><label class="form-label">Observaciones</label><textarea name="observaciones" class="form-control"></textarea></div>
        </div>
        <div class="modal-footer"><button type="button" class="btn btn-light" data-bs-dismiss="modal">Cancelar</button><button class="btn btn-success">Confirmar cobro</button></div>
      </form>
    </div>
  </div>
</div>
