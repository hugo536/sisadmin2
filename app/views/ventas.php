...
<div class="container-fluid p-4" id="ventasApp"
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 fade-in">
        <div>
            ...
            <p class="text-muted small mb-0 ms-1">Gestión de pedidos de clientes y despachos.</p>
        </div>
                        ...
                        <?php if (!empty($ventas)): ?>
                            <?php foreach ($ventas as $venta): ?>
                                ...
                                <tr data-id="<?php echo (int) ($venta['id'] ?? 0); ?>" data-estado="<?php echo $estado; ?>" class="border-bottom" data-search="<?php echo e(mb_strtolower((string) ($venta['codigo'] ?? '') . ' ' . (string) ($venta['cliente'] ?? ''))); ?>">
                                    ...
                                    <td class="text-end fw-bold text-dark fs-6">S/ <?php echo number_format((float) ($venta['total'] ?? 0), 2); ?></td>
                                    ...
                                    <td class="text-end pe-4">
                                        <div class="d-inline-flex align-items-center gap-1">
                                            <?php if ($estado === 0): ?> 
                                                <button type="button" class="btn btn-sm btn-light text-primary border-0 btn-editar rounded-circle" data-id="<?php echo (int) ($venta['id'] ?? 0); ?>" data-bs-toggle="tooltip" title="Editar Pedido"><i class="bi bi-pencil-square fs-5"></i></button>
                                                <button type="button" class="btn btn-sm btn-light text-success border-0 btn-aprobar rounded-circle" data-id="<?php echo (int) ($venta['id'] ?? 0); ?>" data-bs-toggle="tooltip" title="Aprobar Pedido"><i class="bi bi-check2-circle fs-5"></i></button>
                                                <button type="button" class="btn btn-sm btn-light text-danger border-0 btn-anular rounded-circle" data-id="<?php echo (int) ($venta['id'] ?? 0); ?>" data-bs-toggle="tooltip" title="Anular Pedido"><i class="bi bi-trash fs-5"></i></button>
                                                
                                            <?php elseif ($estado === 2): ?> 
                                                <button type="button" class="btn btn-sm btn-light text-warning border-0 btn-revertir rounded-circle" data-id="<?php echo (int) ($venta['id'] ?? 0); ?>" data-bs-toggle="tooltip" title="Volver a Borrador">
                                                    <i class="bi bi-arrow-counterclockwise fs-5"></i>
                                                </button>

                                                <button type="button" class="btn btn-sm btn-light text-info border-0 btn-despachar rounded-circle" data-id="<?php echo (int) ($venta['id'] ?? 0); ?>" data-bs-toggle="tooltip" title="Despachar Mercadería"><i class="bi bi-truck fs-5"></i></button>
                                                <button type="button" class="btn btn-sm btn-light text-secondary border-0 btn-editar rounded-circle" data-id="<?php echo (int) ($venta['id'] ?? 0); ?>" data-bs-toggle="tooltip" title="Ver Detalle"><i class="bi bi-eye fs-5"></i></button>
                                                <button type="button" class="btn btn-sm btn-light text-danger border-0 btn-anular rounded-circle" data-id="<?php echo (int) ($venta['id'] ?? 0); ?>" data-bs-toggle="tooltip" title="Anular Pedido"><i class="bi bi-trash fs-5"></i></button>
                                                
                                            <?php elseif ($estado === 3 || $estado === 5): ?>
                                                <button type="button" class="btn btn-sm btn-light text-warning border-0 btn-devolucion rounded-circle" data-id="<?php echo (int) ($venta['id'] ?? 0); ?>" data-bs-toggle="tooltip" title="Registrar Devolución"><i class="bi bi-arrow-return-left fs-5"></i></button>
                                                <button type="button" class="btn btn-sm btn-light text-secondary border-0 btn-editar rounded-circle" data-id="<?php echo (int) ($venta['id'] ?? 0); ?>" data-bs-toggle="tooltip" title="Ver Detalle"><i class="bi bi-eye fs-5"></i></button>
                                                
                                            <?php else: ?> 
                                                <button type="button" class="btn btn-sm btn-light text-secondary border-0 btn-editar rounded-circle" data-id="<?php echo (int) ($venta['id'] ?? 0); ?>" data-bs-toggle="tooltip" title="Ver Detalle"><i class="bi bi-eye fs-5"></i></button>
                                            <?php endif; ?>
                                            <button type="button" class="btn btn-sm btn-light text-dark border-0 rounded-circle btn-imprimir-modal" data-id="<?php echo (int) ($venta['id'] ?? 0); ?>" data-bs-toggle="tooltip" title="Imprimir PDF"><i class="bi bi-printer fs-5"></i></button>
                                        </div>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
        ...
        <div class="modal-content border-0 shadow-lg">
                ...
                <form id="formVenta" autocomplete="off">
                    ...
                    <div class="card border-info-subtle shadow-sm mt-3 d-none fade-in" id="seccionRegalos">
                        <div class="card-body p-0">
                            ...
                            <div class="table-responsive">
                                <table class="table table-sm align-middle mb-0 table-pro table-pastel" id="tablaDetalleRegalos">
                                    <thead class="table-light border-bottom">
                                        <tr>
                                            <th class="text-center text-secondary col-w-40 py-2">#</th>
            ...
            <div class="modal-footer bg-white border-top-0 d-flex justify-content-between align-items-center flex-wrap gap-2">
                ...
                <div class="d-flex align-items-center gap-2">
                    ...
                    <!-- Añadido Type y Form nativo -->
                    <button type="submit" form="formVenta" class="btn btn-primary px-4 fw-bold" id="btnGuardarVenta">
        ...
        <div class="modal-content border-0 shadow">
            ...
            <div class="modal-header bg-info text-white border-bottom-0 pb-4">
                <h5 class="modal-title fw-bold d-flex flex-wrap align-items-center gap-2">
                    <span><i class="bi bi-truck me-2"></i>Despachar Pedido</span>
                    <small id="despachoClienteNombre" class="fw-semibold text-white-50"></small>
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            ...
            <div class="modal-footer bg-white border-top-0 d-flex justify-content-between align-items-center flex-wrap gap-2">
                ...
                <div class="d-flex align-items-center gap-2">
                    <button type="button" class="btn btn-light text-secondary fw-semibold" data-bs-dismiss="modal">Cancelar</button>
                    <!-- Agregado form="formDespacho" y type="submit" -->
                    <button type="submit" form="formDespacho" class="btn btn-info text-white fw-bold px-4" id="btnGuardarDespacho">
    ...
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content border-0 shadow">
            ...
            <div class="modal-footer bg-white border-top-0">
                ...
                <!-- Agregado form="formDevolucionVenta" y type="submit" -->
                <button type="submit" form="formDevolucionVenta" class="btn btn-warning text-dark fw-bold px-4" id="btnConfirmarDevolucionVenta">
        ...
        <div class="modal-content border-0 shadow-lg">
            ...
            <div class="modal-body p-4 bg-light" style="margin-top: -15px; border-top-left-radius: 1rem; border-top-right-radius: 1rem;">
                ...
                <div>
                    ...
                    <input type="number" class="form-control shadow-none border-secondary-subtle" id="cantidadPaginasPedido" min="1" max="20" step="1" value="1">
