...
function initFormularioRecetas() {
    ...
    // 1. TomSelect Modificado para aceptar Extra Params (como solo_con_bom)
    function initTomSelectAjax(selectEl, placeholderText, extraParams = {}, onChangeCallback = null) {
        ...
        return new TomSelect(selectEl, {
            ...
            load: function(query, callback) {
                ...
                if (extraParams && typeof extraParams === 'object') {
                    Object.keys(extraParams).forEach(key => url.searchParams.set(key, extraParams[key]));
                }
    ...
    // 2. Inicializamos el producto destino CON el filtro de BOM
    if (inputProducto) {
        tomProductoDestino = initTomSelectAjax(inputProducto, 'Seleccione producto a fabricar...', { solo_con_bom: 1 });
    }
    ...
    const crearFilaInsumo = () => {
        ...
        // 3. Inicialización correcta de TomSelect PARA INSUMOS (con {} sin parámetros extra)
 ...
 * SISTEMA SISADMIN2 - Módulo de Producción (Recetas BOM)
 * Optimizado con Tom Select AJAX y SweetAlert2.
 * Versión FINAL CORREGIDA - Costeo Operativo Estándar + SPA Support + Carga de selects corregida
 */
...
function initFormularioRecetas() {
    ...
    window.produccionRecetaFormAPI = {
        ...
        calcularGlobal: calcularResumenYCostos,
        setCabecera(data) {
            if (!data) return;
            const inputCodigo = document.getElementById('newCodigo');
            const inputVersion = document.getElementById('newVersion');
            ...
            if (selectCentroCosto) selectCentroCosto.value = data.id_centro_costo ? String(data.id_centro_costo) : '';
            if (selectAlmacenPlanta) selectAlmacenPlanta.value = data.id_almacen_planta ? String(data.id_almacen_planta) : '';
        ...
        cargarCif(cif) {
            ...
            (cif || []).forEach((fila) => {
                ...
                if (selectConcepto) {
                    selectConcepto.value = fila.id_activo || '';
                }
