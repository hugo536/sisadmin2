...
(function () {
  ...
  // =========================================================
  const closeOpenTomSelects = function() {
  ...
  const handleTomSelectResizeUX = function() {
      // En móviles, al abrir el teclado virtual se dispara resize.
      // Si hay un input interno de TomSelect enfocado, no cerramos para evitar
      // que el teclado se oculte inmediatamente al tocar el campo.
      const activeEl = document.activeElement;
      if (activeEl && typeof activeEl.closest === 'function' && activeEl.closest('.ts-wrapper')) {
          return;
      }
  ...
  document.addEventListener('scroll', handleTomSelectScrollUX, true);

  // Escuchar resize (rotación/cambio de viewport). Ignora resize provocado por teclado móvil.
  window.addEventListener('resize', handleTomSelectResizeUX);
})();
