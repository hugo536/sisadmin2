(function () {
    ...
    function initTableManager() {
        ...
        const filtroDoc = document.getElementById('clienteFiltroDoc');
        const filtroEstado = document.getElementById('clienteFiltroEstado');
        const paginationControls = document.getElementById('clientesPaginationControls');
        const paginationInfo = document.getElementById('clientesPaginationInfo');
        const allRows = Array.from(document.querySelectorAll('#clientesTable tbody tr'));
        ...
        function updatePaginationUI(start, end, totalRows, totalPages) {
            if (paginationInfo) {
                if (totalRows === 0) {
                    ...
                    paginationInfo.textContent = `Mostrando ${start + 1}-${realEnd} de ${totalRows} clientes`;
                }
            }
